...
@Component({
      ...
      @switch (fieldDef().type) {
        ...
        @case ('markdown') {
          <div class="md-wrapper">
            <div class="md-tabs">
              <button type="button" class="md-tab" [class.active]="!showPreview()" (click)="showPreview.set(false)">Editar</button>
              <button type="button" class="md-tab" [class.active]="showPreview()" (click)="showPreview.set(true)">Vista previa</button>
            </div>
            @if (!showPreview()) {
              <textarea
                rows="6"
              ...
              <div class="md-preview" [innerHTML]="renderMarkdown(value() || '')"></div>
            }
          </div>
        }
      }
  ...
  `,
  styles: [`
    .field-wrapper { display: grid; gap: 0.4rem; }
    .field-label {
      font-size: 0.82rem;
      color: var(--text-2);
      font-weight: 500;
    ...
    input:focus, textarea:focus, select:focus { outline: 1px solid var(--accent-glow); }
