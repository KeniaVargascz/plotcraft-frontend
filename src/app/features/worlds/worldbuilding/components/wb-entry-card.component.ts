...
@Component({
    ...
    <article class="entry-card" (click)="select.emit(entry())">
      <div
        ...
        [style.background]="entry().coverUrl
          ...
          : 'linear-gradient(135deg, ' + (entry().category.color || '#2a2a3e') + '22, ' + (entry().category.color || '#2a2a3e') + '55)'"
      >
      ...
      @if (showActions()) {
        <div class="actions-row">
          ...
          <button type="button" class="action-btn danger" (click)="onDelete($event)">Eliminar</button>
        </div>
      }
    </article>
  `,
  styles: [`
    ...
    .entry-card:hover { box-shadow: 0 8px 24px rgba(0,0,0,0.12); }
    .cover {
      position: relative;
      height: 6rem;
      display: grid;
    ...
    .body { padding: 0.85rem 1rem; display: grid; gap: 0.35rem; align-content: start; }
    .cat-row {
      display: flex;
      align-items: center;
      gap: 0.3rem;
    ...
    .tags-row { display: flex; gap: 0.3rem; flex-wrap: wrap; margin-top: 0.15rem; }
    .tag-chip {
      padding: 0.15rem 0.5rem;
      border-radius: 999px;
      border: 1px solid var(--border);
    ...
    .tag-chip.more { background: var(--accent-glow); color: var(--accent-text); border: none; }
    .actions-row {
      display: flex;
      gap: 0.5rem;
    ...
    .action-btn:hover { background: var(--accent-glow); color: var(--accent-text); }
    .action-btn.danger:hover { background: #b42318; color: #fff; }
  `],
})
