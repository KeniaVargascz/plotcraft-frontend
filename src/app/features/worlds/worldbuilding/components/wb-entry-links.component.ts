...
@Component({
        ...
        <div class="links-list">
          @for (link of links(); track link.id) {
            <div class="link-item">
              ...
              @if (isOwner()) {
                <button type="button" class="delete-link-btn" (click)="deleteLink.emit(link.id)">&#10005;</button>
              }
            </div>
          }
        </div>
  ...
  `,
  styles: [`
    .links-panel { display: grid; gap: 0.65rem; }
    .links-header {
      display: flex;
      justify-content: space-between;
      align-items: center;
    }
    .links-header h4 { margin: 0; font-size: 0.92rem; color: var(--text-1); }
    .add-link-btn {
    ...
    .add-link-btn:hover { background: var(--bg-surface); }
    .links-list { display: grid; gap: 0.35rem; }
    .link-item {
      display: flex;
      align-items: center;
      gap: 0.5rem;
    ...
    .link-relation { font-size: 0.7rem; color: var(--text-3); }
    .link-name {
      font-size: 0.82rem;
      color: var(--text-1);
      font-weight: 500;
    ...
    .delete-link-btn:hover { background: #b4231822; color: #b42318; }
    .empty { color: var(--text-3); font-size: 0.82rem; margin: 0; }
  `],
})
