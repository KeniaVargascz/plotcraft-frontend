...
@Component({
    ...
    @if (loading()) {
      ...
      <section class="detail-shell">
        ...
        <header class="hero card">
          @if (entry()!.coverUrl) {
            <div class="hero-cover" [style.backgroundImage]="'url(' + entry()!.coverUrl + ')'"></div>
          }
          <div class="hero-body">
            <div class="hero-meta">
              <span class="cat-badge" [style.background]="(entry()!.category.color || '#6366f1') + '22'" [style.color]="entry()!.category.color || '#6366f1'">
            ...
            <p class="author-line">
              por <a [routerLink]="['/perfil', entry()!.author.username]">{{ entry()!.author.displayName }}</a>
            </p>
          </div>
        </header>
          ...
          <aside class="side-col">
            ...
            @if (entry()!.viewerContext?.isOwner) {
              <div class="card owner-actions">
                <a class="action-btn primary" [routerLink]="['/mis-mundos', entry()!.world.slug, 'world-building', entry()!.category.slug, entry()!.slug, 'editar']">
                  Editar entrada
                </a>
              </div>
            }
  ...
  styles: [`
    ...
    .hero-body { padding: 1.25rem; display: grid; gap: 0.5rem; }
    ...
    .summary { color: var(--text-2); margin: 0; font-size: 0.95rem; line-height: 1.5; }
    ...
    .author-line { color: var(--text-3); font-size: 0.82rem; margin: 0; }
    .author-line a { color: var(--accent-text); text-decoration: none; }
    .content-grid {
      display: grid;
    ...
    h2 { margin: 0 0 0.5rem; font-size: 1.1rem; color: var(--text-1); }
    h3 { margin: 0 0 0.5rem; font-size: 1rem; color: var(--text-1); }
    .fields-grid {
      display: grid;
      gap: 0.5rem;
    }
    ...
    .field-label { font-size: 0.82rem; color: var(--text-3); font-weight: 500; }
    .field-value { font-size: 0.85rem; color: var(--text-1); }
    .prose { line-height: 1.7; color: var(--text-1); font-size: 0.9rem; }
    .side-col { display: grid; gap: 1rem; align-content: start; }
    .owner-actions { display: grid; gap: 0.5rem; }
    .action-btn {
      display: block;
      text-align: center;
      padding: 0.7rem 1rem;
...
export class WbEntryDetailPageComponent {
  ...
  fieldEntries(): Array<{ key: string; label: string; value: any }> {
    const e = this.entry();
